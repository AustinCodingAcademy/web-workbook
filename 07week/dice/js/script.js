...
function report() {
  ...
  for (let dieVal = 1; dieVal < 7; dieVal++) {  // we set up a nested loop, first we check for all the 1s, then 2s, etc
    ...
    if (howManyDice === 5) { // we check after the counting is done if it is equal to '2'
      document.querySelector("#report").innerHTML += `YACHT!!! You have five ${dieVal}s<br>`;  // if yes, we report a pair
    } else if (howManyDice === 4) {
      document.querySelector("#report").innerHTML += `Four of a kind, you have four ${dieVal}s<br>`;
    } else if (howManyDice === 3) {
      document.querySelector("#report").innerHTML += `Three of a kind, you have three ${dieVal}s<br>`;
    } else if (howManyDice === 2) {
      document.querySelector("#report").innerHTML += `Pair, you have two ${dieVal}s<br>`;
    } else if (howManyDice === 2, howManyDice === 2) {
      document.querySelector("#report").innerHTML += `Two Pairs, you have two ${dieVal}s<br>`;
    }
  }
}
