...
function report() {
  ...
  for (let dieVal = 1; dieVal < 7; dieVal++) { // we set up a nested loop, first we check for all the 1s, then 2s, etc
    ...
    if (howManyDice === 5) {
      document.querySelector("#report").innerHTML += `There are a Yatch of ${dieVal}s<br>`; // if yes, we report a yatch
    }

  }
  if (howManyPairs === 2) {
    document.querySelector("#report").innerHTML += `There are two-pairs at once`;
  }

  if (howManyPairs === 1 && howManyThrees === 1) {
  ...
  for (let dieVal = 1; dieVal < 6; dieVal++) {
    ...
    for (let i = 0; i < 5; i++) { // we create the inner loop that cycles through the rolled dice
      if (dieVal === Number(diceHtml[i].getAttribute('data-roll'))) { // we check if the rolled die is equal to the one's we're counting
        found = 1; // if so, we add one to the count
      }
    }
    if (found === 0) {
      break;
  ...
  if (found === 1) {
    document.querySelector("#report").innerHTML += `This is a small-straight`;
  }



  for (let dieVal = 2; dieVal < 7; dieVal++) {
    found = 0;
    for (let i = 0; i < 5; i++) { // we create the inner loop that cycles through the rolled dice
      if (dieVal === Number(diceHtml[i].getAttribute('data-roll'))) { // we check if the rolled die is equal to the one's we're counting
        found = 1; // if so, we add one to the count
  ...
  if (found === 1) {
    document.querySelector("#report").innerHTML += `This is a big-straight`;
  }
