...
echo "<title>USGS Research - PAHs and Coal-Tar-Based Sealcoat Photo Description</title>\n";
?>
...
if ((string) $thispicture->filename == $base) {


echo "<h1>USGS Research: PAHs and Coal-Tar-Based Pavement Sealcoat<br>Photo details: " . htmlentities($thispicture->title) . "</h1>\n<hr>\n";	
...
<tr align="center" ><td style="">	
<div class="photo">
<?php
if (file_exists($largeimage)){
echo "<a href=\"$largeimage\"><img src=\"".$base."_med.jpg\" ".  $attr ." alt=\"$alt\" title=\"$alt\" ></a>";
}
else {
echo "<img src=\"".$base."_med.jpg\" ".  $attr ." alt=\"$alt\" title=\"$alt\" >";
}
?>        
 <p><?php echo  htmlentities($thispicture->longdescription); ?></p>
</div>


    <div class="details">
    <?php
		$locate = htmlentities($thispicture->location);
		if ($locate != ""){
        	echo "<p><span class=\"bold\">Location</span>: $locate</p>\n";
        }
		$picdate = htmlentities($thispicture->date);
		if ($picdate != ""){
        	echo "<p><span class=\"bold\">Date</span>: $picdate</p>\n";
        }
		echo "<p>";
		$person = htmlentities($thispicture->photographer);
		if ($person != ""){
        	echo "<span class=\"bold\">Photographer</span>: $person";	
        }
		$affil = htmlentities($thispicture->affiliation);
			if ($affil == "CLEARCorps") {
				echo "&copy;CLEARCorps (Used with permission)"; 
				$download="no";}
			elseif ($affil != "") {
				echo ", $affil";
				if ($affil != "USGS") {echo " (Used with permission)";}
			}
		echo "</p>";
	...
	if ($download != "no") {
		...
		echo "<p style=\"margin-bottom:0em\">Download ";
        
		

		if (file_exists($thumbnail)) {
			list($width, $height, $type, $attr) = getimagesize($thumbnail);	
			echo "<a href=\"$thumbnail\">Thumbnail</a> ($width x $height)\n";
		}
		if (file_exists($image)) {
			list($width, $height, $type, $attr) = getimagesize($image);	
			echo "  |  <a href=\"$image\">Medium</a> ($width x $height)\n";
		}
		if (file_exists($largeimage)) {
			list($width, $height, $type, $attr) = getimagesize($largeimage);	
			echo "  |  <a href=\"$largeimage\">Large</a> ($width x $height)\n";
		}
		?>
        </p>
        <p style="margin-top:0em">Contact <a href="mailto:bjmahler@usgs.gov?subject=Sealcoat+Photo+Request">Barbara Mahler</a> to request a high-resolution copy of this photograph.</p>
