...
//can't put large block on top of small one
//to win move all items in same order to last column
//+3 column
// start with 3 blocks
//every level goes up one block
//blocks should be muvable, left and righ//
...
$(function() {
  ...
  function droppable($stack, $detached) {
    ...
    if (
      parseInt($detached.attr("data-block")) <
        parseInt($last_block.attr("data-block")) ||
  ...
  function resetGame() {
    $('[data-stack="1"]').html(
      '<div data-block="100"></div><div data-block="75"></div><div data-block="50"></div><div data-block="25"></div>'
    );
    $('[data-stack="2"]').empty();
    $('[data-stack="3"]').empty();
