 ...
 let triple = 0;
  ...
  for (var dieVal = 1; dieVal < 7; dieVal++) {  // we set up a nested loop, first we check for all the 1s, then 2s, etc
   ...
   pair++;
      ...
      document.getElementById("report").innerHTML += "There are a three of "+dieVal+"s<br>";  // if yes, we report a pair
   triple++;
    }
        if (howManyDice===4) { // we check after the counting is done if it is equal to '2'
      document.getElementById("report").innerHTML += "There are a four of "+dieVal+"s<br>";  // if yes, we report a pair
    }
        if (howManyDice===5) { // we check after the counting is done if it is equal to '2'
      document.getElementById("report").innerHTML += "There are a yacht of "+dieVal+"s<br>";  // if yes, we report a pair
    }
  }
  if (pair===2) {
  ...
  }  else if (pair===0 && triple===0 && four===0 && yacht===0)
  { document.getElementById("report").innerHTML += "Straight";
  }
