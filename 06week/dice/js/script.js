...
function report() {
  ...
  console.log(diceHtml);// this puts all the individual die HTML elements in diceHtml
  for (var dieVal = 1; dieVal < 7; dieVal++) {  // we set up a nested loop, first we check for all the 1s, then 2s, etc
    let howManyDice = 0;
    let pairs = 0;// we zero out our counter
    ...
    if (pairs===1 && howManyDice===3) {
      document.getElementById("report").innerHTML += "You have a full house!<br>";
    }
  }
