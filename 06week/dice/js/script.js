...
function report() {
  ...
  for (let dieVal = 1; dieVal < 7; dieVal++) {  // we set up a nested loop, first we check for all the 1s, then 2s, etc
    ...
    if (howManyDice === 3) { // we check after the counting is done if it is equal to '3'
      document.querySelector("#report").innerHTML += `There are three ${dieVal}s<br>`;  // if yes, we report a three
    }
    if (howManyDice === 4) { // we check after the counting is done if it is equal to '2'
      document.querySelector("#report").innerHTML += `There are four ${dieVal}s<br>`;  // if yes, we report a four of kind
    }
