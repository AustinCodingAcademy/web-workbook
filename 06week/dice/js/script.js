...
//* This is the new code that was added to try to understand it: *//
  let pair = 0; 
  let house = 0;
  let straight = [];
  for (var dieVal = 1; dieVal < 7; dieVal++) {  // we set up a nested loop, first we check for all the 1s, then 2s, etc
    ...
    for (var i = 0; i < 5; i++) {  // we create the inner loop that cycles through the rolled dice
    ...
    house++;
    } else if (howManyDice === 4) { // we check after the counting is done if it is equal to '4'
  ...
  if (pair === 2) {
    document.getElementById("report").innerHTML += "This is two-pairs.";
  }
  if ((pair === 1) && (house === 1)) {
    document.getElementById("report").innerHTML += "This is full-house.";
  }
  straight.sort();
  straight.toString();
  if (straight == "1,2,3,4,5") {
    document.getElementById("report").innerHTML += "This is a rare small-straight.";
  }
  if (straight == "2,3,4,5,6") {
    document.getElementById("report").innerHTML += "This is a rare big-straight.";
  }
