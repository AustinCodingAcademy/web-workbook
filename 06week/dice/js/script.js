...
function report() {
    ...
    for (let dieVal = 1; dieVal < 7; dieVal++) {  // we set up a nested loop, first we check for all the 1s, then 2s, etc
        ...
        if (howManyDice === 2) { // we check after the counting is done if it is equal to '2'
            ...
            document.querySelector("#report").innerHTML += `There are a three of ${dieVal}s<br>`;
        }
        else if (howManyDice === 4) {
            document.querySelector("#report").innerHTML += `There are a four of ${dieVal}s<br>`;
        }
        else if (howManyDice === 5) {
            document.querySelector("#report").innerHTML += `There are a yacht of ${dieVal}s<br>`;
        }
    }
    var pairCount = 0;
    var pairNumber = null;
    for (let i=0; i < 5; i++) {
        for (let j =i +1; j <5; j++) {
            if (Number(diceHtml[i].getAttribute('data-roll')) === Number(diceHtml[j].getAttribute('data-roll'))){
                pairCount++;
                pairNumber= Number(diceHtml[i].getAttribute('data-roll'));
            } else if (pairNumber !== Number(diceHtml[i].getAttribute('data-roll')) &&
                Number(diceHtml[i].getAttribute('data-roll')) === Number(diceHtml[j].getAttribute('data-roll'))) {
