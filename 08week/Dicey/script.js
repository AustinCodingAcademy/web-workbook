...
function report() {
  ...
  for (let dieVal = 1; dieVal <= 6; dieVal++) { // we set up a nested loop, first we check for all the 1s, then 2s, etc
    let howManyDice = 0; // we zero out our counter
    for (let die = 0; die < 5; die++) { // we create the inner loop that cycles through the rolled dice
